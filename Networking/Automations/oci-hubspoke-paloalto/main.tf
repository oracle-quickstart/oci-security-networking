...
resource "oci_core_route_table" "fw_route_table" { 
 depends_on = [ oci_core_instance.palo_alto, oci_core_vnic_attachment.trust_vnic ]
 compartment_id = var.compartment_id
 vcn_id         = oci_core_vcn.HUB-VCN.id
 display_name   = "fw_route_table"
 route_rules {
 network_entity_id = data.oci_core_private_ips.pan_trust_subnet_private_ip.private_ips[0].id
...
resource "oci_core_subnet" "pan_mgmt_subnet" {
  depends_on = [ oci_core_security_list.hub_security_list ]
  compartment_id      = var.compartment_id
  cidr_block          = var.pan_mgmt_subnet
  vcn_id              = oci_core_vcn.HUB-VCN.id
  display_name        = "pan_mgmt_subnet"
  dns_label           = "panmgmtsubnet"
...
resource "oci_core_subnet" "pan_untrust_subnet" {
  compartment_id      = var.compartment_id
  vcn_id              = oci_core_vcn.HUB-VCN.id
  display_name        = "pan_untrust_subnet"
  dns_label           = "panuntrust"
  cidr_block          = var.pan_untrust_subnet
  availability_domain = data.oci_identity_availability_domains.ads.availability_domains[0].name
}

resource "oci_core_subnet" "pan_trust_subnet" {
  compartment_id      = var.compartment_id
  vcn_id              = oci_core_vcn.HUB-VCN.id
  display_name        = "pan_trust_subnet"
  dns_label           = "pantrustsubnet"
  cidr_block          = var.pan_trust_subnet
...
data "oci_core_private_ips" "pan_trust_subnet_private_ip" { 
  depends_on = [ oci_core_instance.palo_alto, oci_core_vnic_attachment.trust_vnic ]
